...
router.post('/video', async (req, res) => {
    ...
    const tempFileUrl = 'backend/tmp/video.mp4'

    exec(`yt-dlp --remux-video mp4 -S "res:720,codec:h264" -o ${tempFileUrl} ${videoUrl}`, async (error, stdout, stderr) => {  //download youtube video into temp storage //-f mp4 (--format mp4) forces .mp4 extension
        if (error) {
        ...
        try {
            await uploadVideo(tempFileUrl) //Wait for upload to complete before responding
                res.json({
...
const uploadVideo = async (filePath: string) => {
    ...
    const {data, error} = await supabase
        ...
        .upload('temp/video.mp4', fileBuffer, {contentType:'video/mp4'});
